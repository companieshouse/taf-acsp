...
test("Verify only active companies can register as ACSPs, @smoke @limited @StopScreen", async () => {
  ...
  await companyNumberPageContext.enterCompanyNumber(
  ...
  await assertionsContext.checkElementNotVisible(
    companyNotActivePageContext.page.getByRole("button", { name: "continue" })
  );
});
