...
test.beforeEach("Log in to use ACSP Service", async ({ page }) => {
  ...
  businessMustBeAMLRegisteredPageContext = new BusinessMustBeAMLRegistered(
    page
  );
  await setUp.ACSPUserLogin();
});

test("Verify only business' AML registered with the business name can register as ACSPs, @smoke @limited @StopScreen", async () => {
  ...
  await assertionsContext.checkElementNotVisible(
    businessMustBeAMLRegisteredPageContext.page.getByRole("button", {
      name: "continue",
    })
  );
});
